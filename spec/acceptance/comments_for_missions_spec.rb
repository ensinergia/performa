...
feature "Comments for visions:", :js => true do
  ...
  describe "Given there exists a mission for a given company" do
     
    before(:each) do
      @mission = Factory(:mission_with_assoc, :company => @user.company)
    ...
    describe "and one comment" do
      before(:each) do
        @comment = Factory(:comment, :content => 'My comment', :commentable => @mission, :user => @user)
        visit @sub_host + creed_missions_path
      end
